...
showModalEliminarMovimiento():void{
...
//******************************** AGREGAR INSUMO ************************************
//************************************************************************************
insumoForm={cod_insumo:0,nombre:'',medida:0,existencia:0,descripcion:'',vender_negativo:0,habilitar_stock:0,estado:0, minimo:0};
movimientoInsumoForm={cod_mov:0, movimiento:1,precio:0,ps_precio:0,tipo_cambio:null as number | null, ci_usuario:0, descripcion:'',fecha:'',hora:'',estado:0,origen:0};
detalleMovInsumoForm={lote:0,cod_mov:0,cod_insumo:0,nombre:'',nom_medida:'', precio:null as number | null, precio_unidad:0,cantidad:null as number | null,existencia:0,existencia_actual:0};
...
Agregar_Insumo(){
  ...
  if ((this.movimientoInsumoForm.tipo_cambio ?? 0) > 0) {
    this.detalleMovInsumoForm.precio=Number((this.detalleMovInsumoForm.precio ?? 0) * (this.movimientoInsumoForm.tipo_cambio ?? 0) );
  }else{
    this.detalleMovInsumoForm.precio=Number(this.detalleMovInsumoForm.precio);
  }

  //Agregamos los datos faltantes de movimiento_insumo principal
  this.movimientoInsumoForm.cod_mov=this.ultimo_cod_mov+1;
  this.movimientoInsumoForm.precio=0;
  this.movimientoInsumoForm.fecha=this.transformar_Fecha(this.getTodayDate());
  this.movimientoInsumoForm.hora=this.obtener_hora();
  this.detalleMovInsumoForm.lote = this.ultimo_lote+1; 
  this.ultimo_lote++;
  this.detalleMovInsumoForm.cod_mov = this.movimientoInsumoForm.cod_mov;
  this.detalleMovInsumoForm.precio_unidad = (this.detalleMovInsumoForm.cantidad || 0) / (this.detalleMovInsumoForm.cantidad || 0);
  this.detalleMovInsumoForm.existencia = this.detalleMovInsumoForm.cantidad || 0; 
  this.detalleMovInsumoForm.existencia_actual = this.detalleMovInsumoForm.cantidad || 0; 
  this.detalleMovInsumoForm.precio_unidad=Number((Number(this.detalleMovInsumoForm.precio)/Number(this.detalleMovInsumoForm.cantidad)).toFixed(3));
  //Agregamos el ci_usuario
  const ci_usuario=localStorage.getItem('ci_usuario') 
  if (ci_usuario!==null) {
    this.movimientoInsumoForm.ci_usuario=JSON.parse(ci_usuario);
  }

  //OBTENEMOS EL ULTIMO INSUMO
  this.insumoForm.cod_insumo=this.getlast_CodInsumo();
  //AGREGAMOS EL INSUMO AL MOVIMIENTO
  this.detalleMovInsumoForm.cod_insumo = this.insumoForm.cod_insumo;

  console.log("Insumo Agregar: ",this.insumoForm);
  console.log("Movimiento: ", this.movimientoInsumoForm);
  console.log("Detalle Movimiento: ", this.detalleMovInsumoForm);
  const data = [this.insumoForm, this.movimientoInsumoForm, this.detalleMovInsumoForm]
  this.insumoService.post_InsumoApi(data)
  .subscribe(
    res => {
...
cambiarEstado_Insumo(){
  ...
  .subscribe(
    res => {
      console.log("Insumo Modificado");
      this.mostrarToast('Estado del insumo modificado', 'verde');
      this.listar_Insumos();
    ...
    err => {
      ...
      this.mostrarToast('Error al modificar estado del insumo', 'rojo');
    }
  ) 
}
...
limpiar_Datos(){
  this.insumoForm={cod_insumo:0,nombre:'',medida:0,existencia:0,descripcion:'',vender_negativo:0,habilitar_stock:0,estado:0,minimo:0};
}
...
Registrar_Movimiento(){
  ...
  .subscribe(
    ...
    err => {
      ...
      this.mostrarToast('Error al agregar el movimiento', 'rojo');
    }
  ) 
...
modificar_Movimiento(){
  ...
  .subscribe(
    ...
    err => {
      ...
      this.mostrarToast('Error al modificar el movimiento', 'rojo');
    }
  ) 
  // console.log("Movimiento Agregar: ", this.movimientoForm);
...
eliminar_Movimiento(){
  // this.movimientoForm
  console.log("this.movimientoForm.cod_mov: ",this.movimiento_Anterior.cod_mov);
  
    //REGISTRAR MOVIMIENTO
    this.insumoService.eliminar_Movimiento_y_Detalles(this.movimiento_Anterior.cod_mov)
    .subscribe(
      res => {
        ...
        this.limpiar_datos_movimiento();
        $("#modalEliminarMovimiento").modal('hide');
      },
      err => {
        console.log('Error al agregar Insumo');
...
filtrarLista(){
  ...
  if (this.opcionSeleccionada === 3) {   
    ...
    this.lista_insumos_filtrados = this.listaInsumos.filter((producto: { minimo: any, existencia: any; }) => producto.minimo >= producto.existencia);
  }
...
//******************* DIFERENTES FONDOS PARA LA LISTA MOVIMIENTOS DETALLES TODO **********************
...
isFormInsumoValid():boolean{
  ...
  const minimo = this.insumoForm.minimo || 0;
  const cantidad = this.detalleMovInsumoForm.cantidad || 0; const precio = this.detalleMovInsumoForm.precio || 0; const descripcionMov = this.movimientoInsumoForm.descripcion || '';
  ...
  // return false;
  return validarInsumo && validarMovimiento;
}

isFormInsumoValidMod():boolean{
  const nombre = this.insumoForm.nombre ||''; console.log(nombre);
  const descripcion = this.insumoForm.descripcion || '';
